...
void equivalencematrix() {
  ...
  for (auto x: umap) {
    cout << x.first << " ";
    for (auto y: umap) {
      if (umap[x.first] == umap[y.first] && umap[x.first] != "struct" && umap[x.first] != "array")
        cout << "Y" << " ";
  ...
  */
    while(1)
    {
        cout << "Enter the space seperated arguments\n";
        string s;
        cin >> s;
        string a = "",b = "";
        int flag = 0;
        ...
        cout << ans_A << endl;
        cout << ans_B << endl;
        if(ans_A == ans_B && ans_A == "struct")
        {
            if(struct_structural_equivalence(a, b) == "Y")
                cout << "True as " << s << " are structurally equivalent.\n";
            else
                cout << "False as " << s << " are not structurally equivalent.\n";
        }
        else if(ans_A == ans_B && ans_A == "array")
        {
            if(array_structural_equivalence(a, b) == "Y")
                cout << "True as " << s << " are structurally equivalent.\n";
            else
                cout << "False as " << s << " are not structurally equivalent.\n";
        }
        else if(ans_A == ans_B && ans_A == "function")
        {
            if(return_val[a] == return_val[b] && argument_val[a] == argument_val[b])
		  {
              if(function_equivalence(a,b) == "Y")
			    cout << "True as " << s << " are structural equivalent.\n";
                else
                {
                    cout << "False as "<< s << " are not structurally equivalent1.\n";
                }
		  }
		  else
		  {
			  cout << "False as " << s << " are not structurally equivalent2.\n";
		  }
        }
        else if(ans_A == ans_B)
        {
            cout << "True as " << s << " are name equivalent.\n";
            for(int k = 0; k < inte.size(); k++)
            {
                //cout << inte[k].first << " " << inte[k].second << endl;
                if(inte[k].first == a && inte[k].second == b || inte[k].first == b && inte[k].second == a)
                    f = 1;
            }
            if(f)
            {
                cout << "True as " << s << " are internal name equivalent.\n";
            }
            else
            {
                   cout << "False as " << s << " are not internal name equivalent.\n";
            }
        }
        else
        {
            cout << "False as " << s << " are not name equivalent3.\n";
        }
    }
}
